...
import random
import lib.network
from transmomo.lib.loss import *
from transmomo.lib.util.general import weights_init, get_model_list, get_scheduler
from transmomo.lib.network import Discriminator
...
class BaseTrainer(nn.Module):
    def __init__(self, config):
        ...
        self.ae_opt = torch.optim.Adam(
            [p for p in ae_params if p.requires_grad],
            lr=lr,
            betas=(beta1, beta2),
            weight_decay=config.weight_decay,
        )
    ...
    def save(self, snapshot_dir, iterations):
        ...
        dis_name = os.path.join(
            snapshot_dir, "discriminator_%08d.pt" % (iterations + 1)
        )
        ...
        torch.save(
            {
                "autoencoder": self.ae_opt.state_dict(),
                "discriminator": self.dis_opt.state_dict(),
            },
            opt_name,
        )
    ...
    def evaluate(cls, autoencoder, data, config):
        ...
        with torch.no_grad():  # 2D eval
            ...
            re_dict["loss_val_recon_x"] = cls.recon_criterion(
                x_a_recon, x_a
            ) + cls.recon_criterion(x_b_recon, x_b)
            re_dict["loss_val_cross_body"] = cls.recon_criterion(
                x_aba_recon, x_aba
            ) + cls.recon_criterion(x_bab_recon, x_bab)
            re_dict["loss_val_total"] = (
                0.5 * re_dict["loss_val_recon_x"] + 0.5 * re_dict["loss_val_cross_body"]
            )
...
class TransmomoTrainer(BaseTrainer):
    ...
    def dis_update(self, data, config):
        ...
        angles += (
            self.angle_unit * self.rotation_axes * torch.randn([3], device=x_a.device)
        )
        angles = angles.unsqueeze(0).unsqueeze(2)  # [B=1, K, T=1, 3]

        X_a_recon = self.autoencoder.decode(motion_a, body_a, view_a)
        x_a_trans = rotate_and_maybe_project(
            X_a_recon,
            angles=angles,
            body_reference=config.autoencoder.body_reference,
            project_2d=True,
        )

        x_a_exp = x_a.repeat_interleave(config.K, dim=0)

        self.loss_dis_trans = self.discriminator.calc_dis_loss(
            x_a_trans.detach(), x_a_exp
        )

        if config.trans_gan_ls_w > 0:
            X_s_recon = self.autoencoder.decode(motion_s, body_s, view_s)
            x_s_trans = rotate_and_maybe_project(
                X_s_recon,
                angles=angles,
                body_reference=config.autoencoder.body_reference,
                project_2d=True,
            ...
            self.loss_dis_trans_ls = self.discriminator.calc_dis_loss(
                x_s_trans.detach(), x_s_exp
            )
        else:
            self.loss_dis_trans_ls = 0

        self.loss_dis_total = (
            config.trans_gan_w * self.loss_dis_trans
    ...
    def ae_update(self, data, config):
        ...
        # invariance loss
        self.loss_inv_v_ls = (
            self.recon_criterion(view_a, view_s) if config.inv_v_ls_w > 0 else 0
        )
        self.loss_inv_m_ls = (
            self.recon_criterion(motion_a, motion_s) if config.inv_m_ls_w > 0 else 0
        )
        ...
        # reconstruction
        X_a_recon = self.autoencoder.decode(motion_a, body_a, view_a)
        x_a_recon = rotate_and_maybe_project(
            X_a_recon,
            angles=None,
            body_reference=config.autoencoder.body_reference,
            project_2d=True,
        ...
        X_s_recon = self.autoencoder.decode(motion_s, body_s, view_s)
        x_s_recon = rotate_and_maybe_project(
            X_s_recon,
            angles=None,
            body_reference=config.autoencoder.body_reference,
            project_2d=True,
        ...
        x_sa_recon = rotate_and_maybe_project(
            X_sa_recon,
            angles=None,
            body_reference=config.autoencoder.body_reference,
            project_2d=True,
        ...
        self.loss_cross_x = 0.5 * self.recon_criterion(
            x_as_recon, x_s
        ) + 0.5 * self.recon_criterion(x_sa_recon, x_a)
        ...
        self.loss_gan_trans_ls = (
            self.discriminator.calc_gen_loss(x_s_trans)
            if config.trans_gan_ls_w > 0
            else 0
        )
        ...
        self.loss_inv_m_trans = 0.5 * self.recon_criterion(
            motion_a_trans, motion_a.repeat_interleave(config.K, dim=0)
        ) + 0.5 * self.recon_criterion(
            motion_s_trans, motion_s.repeat_interleave(config.K, dim=0)
        )
        self.loss_inv_b_trans = 0.5 * self.recon_criterion(
            body_a_trans, body_a.repeat_interleave(config.K, dim=0)
        ) + 0.5 * self.recon_criterion(
            body_s_trans, body_s.repeat_interleave(config.K, dim=0)
        )
        ...
        # add all losses
        self.loss_total = torch.tensor(0.0).float().cuda()
