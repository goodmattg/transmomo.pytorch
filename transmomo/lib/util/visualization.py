...
from transmomo.lib.util.motion import normalize_motion_inv, globalize_motion
from transmomo.lib.util.general import ensure_dir
...
def two_pts_to_rectangle(point1, point2):
    ...
    if beta <= -180:
        beta += 360
    p1 = (
        int(point1[0] - length * math.cos(math.radians(beta))),
        int(point1[1] - length * math.sin(math.radians(beta))),
    )
    p2 = (
        int(point1[0] + length * math.cos(math.radians(beta))),
        int(point1[1] + length * math.sin(math.radians(beta))),
    ...
    p3 = (
        int(point2[0] + length * math.cos(math.radians(beta))),
        int(point2[1] + length * math.sin(math.radians(beta))),
    )
    p4 = (
        int(point2[0] - length * math.cos(math.radians(beta))),
        int(point2[1] - length * math.sin(math.radians(beta))),
...
def visualize_motion_in_training(
    outputs, mean_pose, std_pose, nr_visual=4, H=512, W=512
):
    ret = {}
