from transmomo.lib.data import get_dataloader
from transmomo.lib.util.general import write_loss, get_config, to_gpu
import lib.trainer
import os
import time
import argparse
...
def parse_args():
    ...
    parser.add_argument(
        "--preload",
        action="store_true",
        help="load all data into memory before training",
    )
...
def train_with_config(config, opts, logger=None):
    ...
    train_writer = tensorboardX.SummaryWriter(
        os.path.join(opts.out_dir, config.name, "logs")
    )
    checkpoint_directory = os.path.join(opts.out_dir, "checkpoints")
    os.makedirs(checkpoint_directory, exist_ok=True)
    shutil.copy(
        opts.config, os.path.join(opts.out_dir, "config.yaml")
    )  # copy config file to output folder

    # Start training
    iterations = (
        trainer.resume(checkpoint_directory, config=config) if opts.resume else 0
    )
        ...
        for it, data in enumerate(train_loader):
            ...
            if (iterations + 1) % config.log_iter == 0:
                if logger is not None:
                    ...
                    logger.log(
                        "training %6d/%6d, elapsed: %.2f hrs"
                        % (iterations + 1, max_iter, elapsed)
                    )
